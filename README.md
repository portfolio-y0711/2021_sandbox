...
* __개념 증명 및 예광탄 개발__: 기존에 진행하던 메인 프로젝트에 여러 기능이 부가되면서부터, 의존성, 디자인 변경에 따른 파급효과가 부담스러워 실험적 시도를이 점차 사라지게 되었습니다. 본 미니 프로젝트는 메인 프로젝트에서 구현하기조 하는 기술의 적용 가능성을 가늠해보는 **_개념 증명_** 의 역할과 다양한 디자인, 코딩 스타일을 실험해 보기 위한 **_샌드박스_** 역할을 수행하기 위해 시작되었습니다. 

* __프로젝트 컨셉 조감도__: 미니 프로젝트는 메인 프로젝트가 CI / CD 운영 환경에서 안정적으로 배포되는 시점까지, 메인 프로젝트의 전체적인 컨셉(사용된 기술, 개발 방법론, 워크플로우)을 요약하여 보여줄 수 있는 최소한의 기능을 가진 축소판으로 지속적으로 개선 보완될 예정입니다. 
...
* __점진적 개발(Incremental Model)__: 사전 설계(Design Up Front) 없이 테스트 코드만으로 점진적으로 개발을 진행해 나갈 예정입니다. 소규모 토이 프로젝트를 반복하면서 실현 불가능한 기획보다는 견고한 테스트 코드와 not to-do-now-list가 유용하다는 것을 깨달았습니다. 
...
#### Redux Middleware의 정확한 내부 동작을 이해하지 못해, dispatch chain을 예측 가능한 형태로   작성하는 것이 어려움 

> 메인 프로젝트에서 react 컴포넌트의 로딩과 사용자 입력에 따른 이벤트들을 command, query, event 세 가지 

* [Middleware 호출 스택이 열리는 순서][googlelink]

* [Middleware 내부 next(action) 호출 구문 위치][googlelink]
